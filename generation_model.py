...
class QwenForCausalLM(nn.Module):
    ...
    def decode_without_cache(self, input_ids, temp, top_k, top_p, max_new_tokens = 512):
        self.eval()
        print(f'Decoding..')
    ...
    def calc_after_rotary_emb(self, q, k, v, device, prefill = False, seq_len = None):
        ...
        k = repeat_kv(k, num_key_value_groups).to(device)
        v = repeat_kv(v, num_key_value_groups).to(device)
    ...
    def prefill_with_cache(self, input_ids, temp, top_k, top_p):
        ...
        return next_token, current_position
    ...
    def generate(self, tokenizer, input_ids, temp, top_k, top_p, use_cache = False, max_new_tokens = 512):
        ...
        if not use_cache:
            with torch.no_grad():
                ...
                #prefill_metrics, 
                input_ids = self.prefill_without_cache(input_ids, temp = temp, top_k = top_k, top_p = top_p)
                #decode_without_cache(self, input_ids, temp, top_k, top_p, max_new_tokens=100)
                #decode without cache
                input_ids = self.decode_without_cache(input_ids, temp = temp, top_k = top_k, top_p = top_p, max_new_tokens = max_new_tokens)
            ...
            with torch.no_grad():
                ...
                #prefill
                next_token, current_position = self.prefill_with_cache(input_ids, temp, top_k, top_p)
                
                #decode
                generated_tokens= self.decode_with_cache(
                    next_token = next_token, current_position = current_position,
                    temp = temp, top_k = top_k, top_p = top_p, max_new_tokens= max_new_tokens)
